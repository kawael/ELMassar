...
const customIcon = (color, zoomLevel) => {
  const size = Math.max(16, Math.min(16, zoomLevel )); // Adjust size based on zoom level
...
const ArrowIcon = (zoomLevel) => {
  const size = Math.max(16, Math.min(32, zoomLevel * 2));
  return new L.DivIcon({
    iconSize: [size, size],
    iconAnchor: [size / 2, size / 2],
    bgPos: [0, 0],
    className: "dir-icon",
    html: `<div class="ripple-container" style="width: ${size}px; height: ${size}px;">
...
function MapUpdater({ polyline, setTooltipDetails }) {
  const map = useMap();
  ...
  useEffect(() => {
    if (polyline && polyline.length > 0) {
      ...
      if (lastPoint) {
        map.flyTo(lastPoint, map.getZoom(), { animate: true });
      }
    }
  ...
  useEffect(() => {
    if (polyline && polyline.length > 1) {
      ...
      if (!directionMarker) {
        ...
        directionMarker.setIcon(ArrowIcon( zoomLevel));
        setTooltipDetails((prevDetails) => [
          ...prevDetails,
      ...
      <div className="flex-1">
        ...
        >
          ...
          {Object.keys(optionsMap).map((targetId) => (
            optionsMap[targetId] && (
              <div key={targetId}>
                <MapUpdater polyline={polylines[targetId] || []} setTooltipDetails={setTooltipDetails} />
                ...
                {(polylines[targetId] || []).map((position, index, array) =>
                  index < array.length - 1 ? (
                    <Marker
                      key={`${targetId}-${index}`}
                      position={position}
                      icon={customIcon("red", mapRefs.current[targetId]?.getZoom() || 12)}
                    >
                      <Tooltip
                        direction="top"
                        offset={[0, -10]}
